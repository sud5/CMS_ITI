...
function video_status($cmid, $userid){
    global $DB;
    $total_progress = $DB->get_records_sql('SELECT * FROM {video_attempts} WHERE cmid = ? AND userid = ? ORDER BY percentage DESC LIMIT 1', array($cmid, $userid));
...
function videofile_get_completion_state($course, $cm, $userid, $type) {
    ...
    $records = $DB->get_records_sql('SELECT * FROM video_attempts WHERE userid = ? AND percentage >= ?', array($userid, $video->videoprogress));
    if(count($records) > 0){
        $value = true;
    } else {
        $value = false;
 ...
 * @since Moodle 3.0
 */
function videofile_view($videofile, $course, $cm, $context) {
    ...
    $event = \mod_videofile\event\course_module_viewed::create($params);
    $event->add_record_snapshot('course_modules', $cm);
    $event->add_record_snapshot('course', $course);
    $event->add_record_snapshot('videofile', $videofile);
    $event->trigger();
